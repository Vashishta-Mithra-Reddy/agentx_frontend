...
const AddAgentForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // setSuccess('');
    try {
      await axios.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/auth/add-agent`, {
        name, email, password, mobileNumber, countryCode
      });
  ...
  return (
    <div className="flex items-center justify-center w-fit font-outfit">
      <motion.div initial={{y:10,opacity: 0,filter:"blur(5px)"}} animate={{y:0,opacity: 1,filter:"blur(0px)"}} transition={{duration: 0.5,ease:"easeInOut"}} className="px-12 pt-12 pb-6 rounded-xl w-full max-w-md bg-background border-1 border-foreground/20">
        <h2 className="text-3xl mb-12 text-center">Add New Agent</h2>
